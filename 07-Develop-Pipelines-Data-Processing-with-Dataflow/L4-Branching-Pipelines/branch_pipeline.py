...
def run():
    ...
    parser.add_argument('--outputPath', required=True, help='GCS coldline storage bucket')
    ...
    # Branch One - write raw for some reason
    lines | 'WriteRawToGCS' >> beam.io.WriteToText(output_path)

    # Branch Two - Read elements from JSON, filter out individual elements, write to BigQuery
    (lines
        | 'ParseJson' >> beam.Map(parse_json)
        | 'DropField' >> beam.Map(drop_fields)
